...
fn main() -> ! {
    ...
    // For HSI we need to mess with AHB divider (48Mhz is too much? datasheet disagrees but *shrug*)
    let mut config = ch32_hal::Config {
        rcc: hal::rcc::Config::SYSCLK_FREQ_48MHZ_HSI,
        ..Default::default()
    };
    config.rcc.ahb_pre = Hpre::DIV2;
    // This is the default but I still haven't tested with peripherals - TODO confirm.
